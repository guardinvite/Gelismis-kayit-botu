...
exports.run = async (client, message, args) => {
    ...
    let Kız1 = sett.Roller.Kız1
    let Kız2 = sett.Roller.Kız2
    ...
    setTimeout(() => {
    Kullanıcı.roles.cache.has(TagRole) ? Kullanıcı.roles.set([TagRole, Kız1, Kız2]) : Kullanıcı.roles.set([Kız1, Kız2]);
    }, 200) // eğer tag rol'ü varsa türm rolleri alıp tag Kız1 Kız2 atıcak eğer yok ise tüm rolleri alıp Kız1 Kız2 vericek

    db.push(`isimler.${Kullanıcı.id}`, { Olay: "(Kadın kayıt)", Tarih: kayıtbas, Yas: yaş, İsim: isim, Yetkili: message.author.id, Rol1: Kız1, Rol2: Kız2 }) // kayıt olana kullanıcının verilerini kaydettirdik
    db.push(`kayıtlarım.${message.author.id}`, { Olay: "(Kadın kayıt)", Tarih: kayıtbas, Yas: yaş, İsim: isim, User: Kullanıcı.id, Yetkili: message.author.id, Rol1: Kız1, Rol2: Kız2 }) // kullanıcıyı kaydeden yetkiliye de kaydettirdik
    db.add(`kadın.${message.author.id}.kadınkayıt`, 1) // erkek kayıda 1 eklettik
